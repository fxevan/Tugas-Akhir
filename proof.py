...
vsignal = submatrix(fault_signal, 0, 1, 140002, 3)
'''
table = FF.create_table(fault_signal[0:14])
py.plot(table, filename='faultsignal')
table = FF.create_table(fault_signal[139984:140002])
py.plot(table, filename='faultsignal2')

isignal = submatrix(fault_signal, 0, 4, 140002, 3)
trace = go.Scatter(x=[j for j in range(len(isignal))], y=isignal)
py.plot([trace], filename='I signal')
'''
...
trace = go.Scatter(x=[j for j in range(len(cD4))], y=cD4)
py.plot([trace], filename='Wavelet Coefficient')
